\section{Future Work}
While the current demo is functioning, it leaves a lot to be desired, work with this project will continue over the summer in order to improve several aspects. The plan is to finish the transition over to a pure javascript client, removing the Unity WebGL solution that is currently being used. Additionally, the product needs to be made more robust, error \rephrase{retardant}, and user friendly. The target is to reach a level where it is actually presentable to customers.
...
% * Project work will continue during the summer
% * What will be worked on?
%     * Javascript Client
%     * Extending more than 100 users pr room in Photon
%         * Alternative: Rest API for communicating with host
%     * Robustness
%     * Optimizing Latency
%     * Dedicated camera computer
